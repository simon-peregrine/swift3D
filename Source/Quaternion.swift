...
prefix public func +(_ q: quat) -> quat {
    return q
}
...
public func +(_ q: quat, p: quat) -> quat {
    return quat(q.w + p.w, q.x + p.x, q.y + p.y, q.z + p.z)
}

public func *(_ p: quat, q: quat) -> quat {
...
public func *(_ q: quat, v: float3) -> float3 {
...
public func *(_ v: float3, q: quat) -> float3 {
    return inverse(q) * v
}

public func *(_ q: quat, v: float4) -> float4 {
    return float4(q * float3(v), v.w)
}

public func *(_ v: float4, q: quat) -> float4 {
    return inverse(q) * v
}

public func *(_ q: quat, p: Float) -> quat {
    return quat( q.w * p, q.x * p, q.y * p, q.z * p)
}

public func *(_ p: Float, q: quat) -> quat {
    return q * p
}
...
public func rotate(_ q: quat, angle: Float, axis: float3) -> quat {
    var tmp = axis
...
public func angleAxis(_ angle: Float, axis: float3) -> quat {
    var Result = quat()
...
prefix public func +(_ q: dquat) -> dquat {
    return q
}
...
public func +(_ q: dquat, p: dquat) -> dquat {
    return dquat(q.w + p.w, q.x + p.x, q.y + p.y, q.z + p.z)
}

public func *(_ p: dquat, q: dquat) -> dquat {
...
public func *(_ q: dquat, v: double3) -> double3 {
...
public func *(_ v: double3, q: dquat) -> double3 {
    return inverse(q) * v
}

public func *(_ q: dquat, v: double4) -> double4 {
    return double4(q * double3(v), v.w)
}

public func *(_ v: double4, q: dquat) -> double4 {
    return inverse(q) * v
}

public func *(_ q: dquat, p: Double) -> dquat {
    return dquat(q.w * p, q.x * p, q.y * p, q.z * p)
}

public func *(_ p: Double, q: dquat) -> dquat {
    return q * p
}
...
/// Rotates a quaternion from a vector of 3 components axis and an angle.
public func rotate(_ q: dquat, angle: Double, axis: double3) -> dquat {
    var tmp = axis
...
/// Build a quaternion from an angle and a normalized axis.
public func angleAxis(_ angle: Double, axis: double3) -> dquat {
    var Result = dquat()
